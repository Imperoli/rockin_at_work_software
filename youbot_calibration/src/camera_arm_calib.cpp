...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "camera_arm_calib");
  ...
  Eigen::Affine3d T_ee_edge;

  char cmd;
  
  std::cout<<"type for computing frame in camera_frame"<<std::endl;
  std::cin>>cmd;
 ...
 // {
    try
    {
      ...
      tf::transformTFToEigen(transform, T_ee_edge);
      //break;
    }
  ...
  Eigen::Affine3d T_calib_tot(T_calib*T_ee_edge.inverse());
