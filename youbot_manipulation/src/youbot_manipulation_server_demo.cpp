...
bool single_object_cb(std_msgs::String object_name, Eigen::Vector3d& arm_target_gaze, Eigen::Affine3d& T_kinect_armcamera)
{
  Eigen::Affine3d arm_target_pose;
  Eigen::Vector3d arm_target;
  youbot_object_recognition::RecognizeObjectGoal arm_camera_goal;
  float max_probability=0;
  bool acquire_img=true;
  
  /*ac_kinect->waitForResult(); // wait for new kinect action
  ...
  for(int obj=0;obj<kinect_result->object_list.objects.size();obj++)
  {
    
    pose2Affine(kinect_result->object_list.objects[obj].pose.pose, kinect_target_pose);
  //  std::cout<<"kinect pos target\n"<< kinect_target_pose.matrix()<<std::endl;
    ...
    //initial_guess.linear()=initial_guess.linear().inverse();
    
    geometry_msgs::Pose init_guess;
    tf::poseEigenToMsg(initial_guess, init_guess);
    arm_camera_goal.initial_guess_list.push_back(init_guess);
  }*/
  
  arm_camera_goal.acquire_image.data=acquire_img;
  //acquire_img=false; 
  acquire_img=true;
  arm_camera_goal.object_name=object_name;
  ...
  arm_planner::arm_planningGoal arm_goal_grasp;
	...
	ac_arm->waitForResult(); //// wait for previous drop
...
void action_cb(const youbot_manipulation::youbot_manipulationGoalConstPtr & goal)
{
  ...
  bool single_object_succeeded=false;
  for (int i=0; i<goal->object_name_list.size();i++){
    
    single_object_succeeded=single_object_cb(goal->object_name_list[i],arm_target_gaze, T_kinect_armcamera);
    if(!single_object_succeeded) continue;
    
    arm_planner::arm_planningGoal arm_goal_drop;
	  arm_goal_drop.mode=2; //drop target
	  if(goal->object_name_list[i].data.compare("AX-01b_bearing_box")==0)
	  {
	    arm_goal_drop.cartesian_position.x=.4;
	    arm_goal_drop.cartesian_position.y=.25;
	    arm_goal_drop.cartesian_position.z=.0;
	  }
	  else if(goal->object_name_list[i].data.compare("AX-01_bearing_box")==0)
	  {
	    arm_goal_drop.cartesian_position.x=.4;
	    arm_goal_drop.cartesian_position.y=.0;
	    arm_goal_drop.cartesian_position.z=.0;
	  }
	  else
	  {
